...
# Execute the command and always clean up credentials afterward
result=0
aws "${aws_args[@]}" || result=$?
...
if [ ${result} -ne 0 ]; then
  log_error "Sync from source ${SOURCE} to destination ${DESTINATION} failed after ${sync_duration} seconds."
  exit ${result}
fi

log_info "Sync completed successfully in ${sync_duration} seconds."
