...
execute_pgsql_job() {
  ...
  for ((server_idx = 0; server_idx < server_count; server_idx++)); do
    ...
    do
      ...
      if [ "${jobdebug}" = "true" ]; then
        /bin/bash -x dump_pgsql.sh -h "${PGHOST}" -p "${PGPORT}" -U "${PGUSERNAME}" -P "${PGPASSWORD}" -d "${database}" -b "${backuppath}" -f "${filenamedate}" -z "${compress}" >> "${logfile}" 2>&1
        db_result=$?
      else
        /bin/bash dump_pgsql.sh -h "${PGHOST}" -p "${PGPORT}" -U "${PGUSERNAME}" -P "${PGPASSWORD}" -d "${database}" -b "${backuppath}" -f "${filenamedate}" -z "${compress}" >> "${logfile}" 2>&1
        db_result=$?
      fi
    ...
    if check_cron_match "${crontab}"; then
      ...
      if [ "${last_run_minute}" != "${current_minute}" ]; then
        ...
        if ! lockfile -r 0 "${LOCKFILE}" >/dev/null 2>&1; then
          ...
          # Run the job using helper functions
          result=0
