...
# This associative array stores timestamps to prevent duplicate runs within the same minute
...
while true; do
  ...
  # Check each job
  # Note: The loop doesn't need to wake up at exactly :00 seconds.
  # It checks if the current minute matches cron patterns, regardless of the exact second.
  for ((i = 0; i < jobs; i++)); do
    ...
    if check_cron_match "${crontab}"; then
      ...
      # We compare at minute precision to avoid running the same job twice in one minute
      ...
      # This deduplication mechanism ensures jobs run once per matching schedule
      if [ "${last_run_minute}" != "${current_minute}" ]; then
        ...
        # Store current timestamp to prevent duplicate runs in the same minute
  ...
  # This is an optimization - we don't need exact timing, but sleeping to the next minute
  # reduces unnecessary wake-ups. Even if we wake at :03 seconds, we'll correctly 
  # check if the current minute matches any cron patterns.
